...
const FileSystemTree: React.FC<{ parentId: string | null }> = ({ parentId }) => {
  ...
  const { selectedFileIds, setSelectedFileIds, lastSelectedFileId, setLastSelectedFileId } = useEditorContent();
  ...
  const sortedItems = Object.values(files)
    .filter(file => file.parentId === parentId && file.type !== 'papercut')
    ...
    setSelectedFileIds((prevSelectedIds: string[]) => {
      ...
      if (event.ctrlKey || event.metaKey) {
        if (prevSelectedIds.includes(itemId)) {
          ...
          if (clickedItem.type === 'folder') {
            const folderContents = getAllContents(itemId);
            return [...prevSelectedIds, ...folderContents];
          } else {
      ...
      if (event.shiftKey && lastSelectedFileId) {
        ...
        return range.flatMap(item => item.type === 'folder' ? getAllContents(item.id) : [item.id]);
      }
  ...
  }, [files, setSelectedFileIds, setLastSelectedFileId, lastSelectedFileId, sortedItems]);
  ...
  const handleDrop = (e: DragEvent<HTMLDivElement>, targetId: string) => {
    ...
    if (dropIndicator.position === 'inside' && targetItem.type === 'folder') {
      ...
      moveItem(draggedId, targetItem.parentId, dropIndicator.position === 'before' ? targetId : null);
    }
