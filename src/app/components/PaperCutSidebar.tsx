  ...
  const handleItemClick = useCallback((itemId: string, event: React.MouseEvent) => {
    ...
    const updateSelection = () => {
      ...
      if (event.ctrlKey || event.metaKey) {
        setPaperCutSelectedIds(prev => 
          prev.includes(itemId) 
            ? prev.filter(id => id !== itemId)
            : [...prev, itemId]
        );
      } else if (event.shiftKey && paperCutLastSelectedId) {
        const sortedItems = items
          .filter(tab => tab.parentId === parentId && tab.type === 'file')
          .sort((a, b) => a.order - b.order);
        
        const start = sortedItems.findIndex(tab => tab.id === paperCutLastSelectedId);
        const end = sortedItems.findIndex(tab => tab.id === itemId);
        const range = sortedItems
          .slice(Math.min(start, end), Math.max(start, end) + 1)
          .map(tab => tab.id);
        setPaperCutSelectedIds(range);
      } else {
        setPaperCutSelectedIds([itemId]);
      }
    };
        ...
        <React.Fragment key={item.id}>
          <div
            className={`group flex items-center gap-2 px-3 py-2 rounded-md cursor-pointer transition-colors 
              ${paperCutSelectedIds.includes(item.id) ? 'bg-secondary/10' : 'hover:bg-muted/50'}
